...
def show_html(filename, height=900, scrolling=True):
    ...
    Used for 'landing_page.html' and other .html files in the 'templates/' folder.
    """
    try:
        # Build the path from the templates folder
        html_path = pathlib.Path(f"templates/{filename}")
        if not html_path.exists():
            st.error(f"Could not find {filename} in templates/ folder.")
            return
        # Read the entire HTML file text
...
def show_farm_game():
    ...
    try:
        ...
        # Embed the game in a scrolling iframe
...
def main():
    ...
    with tabs[1]:
        ...
        run_arima_forecast()  # from arima_forecast/forecast_page.py
    ...
    with tabs[3]:
        ...
        # Embeds the calculator page from templates/<filename>
        show_html("indexnew.html", height=1200)

    # --- Tab 5: Water Score
    with tabs[4]:
        st.subheader("Water Score Calculator")
        # Embeds the water score page from templates/<filename>
